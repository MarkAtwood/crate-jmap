//! RFC 8620 §5.3 `maxObjectsInSet` enforcement on the filenode-server
//! `FileNode/set` handler (bd:JMAP-ayoz.41.7).
//!
...
use jmap_filenode_server::handle_filenode_set;
use jmap_types::Id;
...
/// Helper: build a `FileNode/set` create-map of `n` entries. The wire
/// shape is independent of `FileNode`'s internal struct — the test
...
fn create_map_of_size(n: usize) -> Value {
    ...
    for i in 0..n {
        map.insert(
            format!("c{i}"),
            json!({"name": format!("node-{i}"), "type": "file"}),
        );
    }
    Value::Object(map)
}
...
fn one_account_backend() -> (MemoryBackend, Id) {
    ...
    let backend = MemoryBackend::new().with_account(account_id.as_ref());
    (backend, account_id)
}

// ---------------------------------------------------------------------------
// FileNode/set — over-limit (501 creates / destroys)
// ---------------------------------------------------------------------------
...
/// storage layer.
#[tokio::test]
async fn filenode_set_over_limit_create_returns_limit_error() {
    let (backend, account_id) = one_account_backend();
    let args = json!({
        "accountId": account_id.as_ref(),
        "create": create_map_of_size(DEFAULT_CAP + 1),
    });
    let err = handle_filenode_set(&backend, &(), args)
        .await
        .expect_err("501-entry FileNode/set create batch must trip maxObjectsInSet cap");
...
#[tokio::test]
async fn filenode_set_over_limit_destroy_returns_limit_error() {
    let (backend, account_id) = one_account_backend();
    let args = json!({
        "accountId": account_id.as_ref(),
    ...
    let err = handle_filenode_set(&backend, &(), args)
        .await
        .expect_err("501-entry FileNode/set destroy batch must trip maxObjectsInSet cap");
    let (etype, edesc) = expected_limit_error_shape();
    assert_eq!(err.error_type.as_str(), etype);
    assert_eq!(err.description.as_deref(), Some(edesc));
}
...
// ---------------------------------------------------------------------------
// FileNode/set — at-limit positive control
// ---------------------------------------------------------------------------
...
/// entries may fail downstream for other reasons but the cap itself
/// MUST NOT reject the request.
#[tokio::test]
async fn filenode_set_at_limit_passes_cap_check() {
    let (backend, account_id) = one_account_backend();
    let args = json!({
        "accountId": account_id.as_ref(),
