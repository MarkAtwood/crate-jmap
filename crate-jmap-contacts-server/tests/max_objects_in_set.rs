//! RFC 8620 §5.3 `maxObjectsInSet` enforcement on the contacts-server
//! `/set` handlers (bd:JMAP-ayoz.41.6).
//!
...
use jmap_contacts_server::{handle_address_book_set, handle_contact_card_set};
use jmap_types::Id;
...
#[tokio::test]
async fn address_book_set_over_limit_create_returns_limit_error() {
    ...
    let err = handle_address_book_set(&backend, &(), args)
        .await
        .expect_err("501-entry AddressBook/set create batch must trip maxObjectsInSet cap");
...
#[tokio::test]
async fn contact_card_set_over_limit_destroy_returns_limit_error() {
    let (backend, account_id) = one_account_backend();
    let args = json!({
        "accountId": account_id.as_ref(),
        "destroy": destroy_array_of_size(DEFAULT_CAP + 1),
    });
    let err = handle_contact_card_set(&backend, &(), args)
        .await
        .expect_err("501-entry ContactCard/set destroy batch must trip maxObjectsInSet cap");
    let (etype, edesc) = expected_limit_error_shape();
    assert_eq!(err.error_type.as_str(), etype);
    assert_eq!(err.description.as_deref(), Some(edesc));
}
...
// ---------------------------------------------------------------------------
// At-limit positive control (AddressBook/set)
// ---------------------------------------------------------------------------
...
/// the request.
#[tokio::test]
async fn address_book_set_at_limit_passes_cap_check() {
    let (backend, account_id) = one_account_backend();
    let args = json!({
        "accountId": account_id.as_ref(),
        "create": create_map_of_size(DEFAULT_CAP),
    });
    let result = handle_address_book_set(&backend, &(), args).await;
