# Agent Instructions — jmap-tasks-server
...
Differences are *only* the spec content (draft-ietf-jmap-tasks-06
...
JMAP Tasks extension method handlers (draft-ietf-jmap-tasks-06). Plugs
into `jmap-server`'s `Dispatcher` via `register_tasks_handlers`. Defines
`TasksBackend` trait; consumers provide the storage impl. A reference
...
jmap-types
    ...
    └── jmap-tasks-types          data types
            └── jmap-tasks-server  ← this crate (sibling of canonical jmap-mail-server)
```
...
TaskList ACLs or per-user scoping. Multi-user production deployments
MUST override the default impl.

The tasks-specific implications: every TaskList / Task mutation must
be authorized against the caller's effective rights on the TaskList
(RFC 9670 `myRights` semantics, propagated through
...
```bash
cargo fmt --all
cargo clippy -p jmap-tasks-server -- -D warnings
cargo test -p jmap-tasks-server
RUSTDOCFLAGS="-D warnings" cargo doc -p jmap-tasks-server --no-deps
```
...
| Dependencies | jmap-types, jmap-tasks-types, jmap-server, serde, serde_json |
| Method names | Must match draft-ietf-jmap-tasks-06 exactly (`TaskList/get`, `Task/set`, `TaskNotification/get`, etc.) |
| Test harness | `MemoryBackend` gated behind `feature = "memory"` — HashMap-based, no external deps. Not production. |

## Subagent Guidance

- Spawn subagents for parallel handler work on independent methods (`task_list.rs` vs `task.rs`)
- Never spawn two subagents editing the same file
- Give each subagent the relevant draft section and the `TasksBackend` trait explicitly
...
- Do not add dependencies beyond: jmap-types, jmap-tasks-types, jmap-server, serde, serde_json
- Do not add auth logic or role checks inside handlers (backend canonical — see section above)
- Do not implement methods not in draft-ietf-jmap-tasks-06 unless explicitly directed
