...
// would force a textual rename of ~150 references across the three /set
...
// diff to one line each (`finalize_set_response::<B, O>(...).await`) and
// leaves the builder refactor as a tractable follow-on.
