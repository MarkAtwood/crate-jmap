# Agent Instructions — jmap-calendars-server
...
Differences are *only* the spec content (draft-ietf-jmap-calendars-26
...
1. Read `PLAN.md` — `CalendarsBackend` trait, module layout, handler pattern
2. Read the relevant draft-ietf-jmap-calendars-26 section for the method you are implementing
...
Plugs into `jmap-server`'s `Dispatcher` via `register_calendars_handlers`.
Defines `CalendarsBackend` trait; consumers provide the storage impl. A
...
jmap-types
    ...
    └── jmap-calendars-types      data types
            └── jmap-calendars-server  ← this crate (sibling of canonical jmap-mail-server)
```
...
- How the `Backend` trait is structured (analog of `CalendarsBackend`)
...
calendar ACLs or per-user availability scoping. Multi-user
production deployments MUST override the default impl.

The calendars-specific implications: every Calendar / CalendarEvent
mutation must be authorized against the caller's effective rights on
the Calendar (RFC 9670 `myRights` semantics, propagated through
...
```bash
cargo fmt --all
cargo clippy -p jmap-calendars-server -- -D warnings
cargo test -p jmap-calendars-server
RUSTDOCFLAGS="-D warnings" cargo doc -p jmap-calendars-server --no-deps
```
...
| Dependencies | jmap-types, jmap-calendars-types, jmap-server, serde_json, thiserror |
| Method names | Must match draft-ietf-jmap-calendars-26 exactly (`Calendar/get`, `CalendarEvent/set`, `CalendarEventNotification/get`, etc.) |
| Test harness | `MemoryBackend` gated behind `feature = "memory"` — HashMap-based, no external deps. Not production. |

## Subagent Guidance

- Spawn subagents for parallel handler work on independent methods (`calendar.rs` vs `event.rs`)
- Never spawn two subagents editing the same file
- Give each subagent the relevant draft section and the `CalendarsBackend` trait explicitly
...
- Do not add dependencies beyond: jmap-types, jmap-calendars-types, jmap-server, serde_json, thiserror
- Do not add auth logic or role checks inside handlers (backend canonical — see section above)
- Do not implement methods not in draft-ietf-jmap-calendars-26 unless explicitly directed
