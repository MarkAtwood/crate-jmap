...
/// `ChatContact/queryChanges` with filter, sort, upToId, and
/// calculateTotal must emit all four optional args on the wire
/// (RFC 8620 §5.6).
///
/// Oracle: draft-atwood-jmap-chat-00 §5 — `blocked` is a valid
/// ChatContact filter field; the comparator shape (property +
/// isAscending) follows RFC 8620 §5.5.
#[tokio::test]
async fn chat_contact_query_changes_with_filter_sort_upto_calculatetotal() {
    let server = MockServer::start().await;
    let resp_body = jmap_response(
        "ChatContact/queryChanges",
        json!({
    ...
    let up_to = Id::from("CC-100");
    sc.chat_contact_query_changes(
        &since,
        None,
    ...
    .await
    .expect("chat_contact_query_changes_with_filter_sort_upto_calculatetotal: must succeed");

    let args = recorded_args(&server).await;
    assert_eq!(
        args["filter"]["blocked"],
        json!(false),
        "filter.blocked must be false"
    );
    assert_eq!(
...
/// `ChatContact/queryChanges` with all None optional args must NOT emit
...
#[tokio::test]
async fn chat_contact_query_changes_all_none_omits_optional_wire_keys() {
    let server = MockServer::start().await;
    let resp_body = jmap_response(
        "ChatContact/queryChanges",
        json!({
    ...
    let sc = helpers::make_client(&server);
    let since = State::from("qs-old");
    sc.chat_contact_query_changes(&since, None, None, None, None, None)
        .await
        .expect("chat_contact_query_changes_all_none_omits_optional_wire_keys: must succeed");
