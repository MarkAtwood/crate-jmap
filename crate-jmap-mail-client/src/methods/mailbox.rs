...
impl super::SessionClient {
    ...
    ///   the matching error list on [`Self::mailbox_get`].
    ...
    ///   patch map (pathological conditions only; see [`Self::email_set`]
    ...
    pub async fn mailbox_set(
        ...
        if create.is_none() && update.is_none() && destroy.is_none() {
            return Err(jmap_base_client::ClientError::InvalidArgument(
                "mailbox_set: at least one of create, update, destroy must be Some \
    ...
    /// - Any transport / protocol variant returned by
    ///   [`JmapClient::call`](jmap_base_client::JmapClient::call) — see
    ///   the matching error list on [`Self::mailbox_get`]. RFC 8620 §5.5
    ///   defines additional /query method-level errors (`anchorNotFound`,
    ///   `unsupportedFilter`, `unsupportedSort`, `tooManyChanges`) that
    ///   surface as
    ///   [`MethodError`](jmap_base_client::ClientError::MethodError).
    ...
    ///   empty-state guard; see [`Self::mailbox_changes`]).
    /// - [`ClientError::InvalidSession`](jmap_base_client::ClientError::InvalidSession)
    ///   if the bound session has no primary account for
    ///   `urn:ietf:params:jmap:mail`.
    /// - Any transport / protocol variant returned by
    ///   [`JmapClient::call`](jmap_base_client::JmapClient::call) — see
    ///   the matching error list on [`Self::mailbox_get`]. RFC 8620 §5.6
