            ...
            for id in id_slice.iter() {
                if id.is_empty() {
                    return Err(jmap_base_client::ClientError::InvalidArgument(
                        "mailbox_get: ids element may not be empty".into(),
                    ));
                }
            }
        }
    ...
    pub async fn mailbox_set(
        ...
        if let Some(ref ids) = destroy {
            for id in ids.iter() {
                if id.is_empty() {
                    return Err(jmap_base_client::ClientError::InvalidArgument(
                        "mailbox_set: destroy element may not be empty".into(),
                    ));
                }
            }
        }
...
mod tests {
    ...
    #[test]
    fn mailbox_get_empty_id_returns_invalid_argument() {
        let ids: &[&str] = &[""];
    ...
    #[test]
    fn mailbox_changes_empty_since_state_returns_invalid_argument() {
        let since_state = "";
    ...
    #[test]
    fn mailbox_query_changes_empty_state_returns_invalid_argument() {
