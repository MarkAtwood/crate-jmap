...
pub async fn handle_chat_set<B: ChatBackend>(
    ...
    if let Some(create_map) = args.get("create").and_then(|v| v.as_object()) {
        for (create_id, obj_val) in create_map {
            ...
            let now: UTCDate = UTCDate::from(now_utc_string().as_str());
    ...
    if let Some(update_map) = args.get("update").and_then(|v| v.as_object()) {
        for (id_str, patch_val) in update_map {
            ...
            match backend
                .update_object::<Chat>(&account_id, &id, patch_val.clone())
                .await
            {
                Ok(_) => {
                    mutated = true;
...
// Suppress unused import warning — UTCDate is used in chat creation above.
#[allow(dead_code)]
fn _check_utcdate(_: UTCDate) {}
