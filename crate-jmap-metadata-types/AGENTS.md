# Agent Instructions — jmap-metadata-types
...
are *only* the spec content (draft-ietf-jmap-metadata-01 here;
...
2. Read the relevant draft-ietf-jmap-metadata-01 section before
...
JMAP Metadata extension data types (draft-ietf-jmap-metadata-01).
Types only — no method handlers. No async. Provides the cross-extension
`Annotation` / `Metadata` companion-object types and the
`MetadataCapability` capability surface. Consumed by
`jmap-metadata-server` and `jmap-metadata-client`.

## Crate Family Context

```
jmap-types
    └── jmap-metadata-types  ← this crate (sibling of canonical jmap-mail-types)
...
```bash
cargo fmt --all
cargo clippy -p jmap-metadata-types -- -D warnings
cargo test -p jmap-metadata-types
RUSTDOCFLAGS="-D warnings" cargo doc -p jmap-metadata-types --no-deps
```
...
| Field names | Must match draft-ietf-jmap-metadata-01 exactly |
...
- Do not add fields or types not present in draft-ietf-jmap-metadata-01
