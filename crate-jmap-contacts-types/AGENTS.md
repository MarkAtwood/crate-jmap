# Agent Instructions — jmap-contacts-types
...
are *only* the spec content (RFC 9610 JMAP Contacts + RFC 9553
JSContact here; RFC 8621 in mail-types).
...
2. Read the relevant RFC 9610 / RFC 9553 section before
...
JMAP Contacts extension data types (RFC 9610). Types only — no
method handlers. No async. Consumed by `jmap-contacts-server` and
`jmap-contacts-client`. Re-exports typed JSContact (RFC 9553)
sub-types via `jmap-jscontact-types` under the `jscontact` module
alias for callers that want to opt into structured value-shapes
for the hybrid Sloppy-Value fields.
...
jmap-types
    ...
    └── jmap-contacts-types  ← this crate (sibling of canonical jmap-mail-types)
...
**Before naming any field or type**, grep the relevant RFC text
...
```bash
cargo fmt --all
cargo clippy -p jmap-contacts-types -- -D warnings
cargo test -p jmap-contacts-types
RUSTDOCFLAGS="-D warnings" cargo doc -p jmap-contacts-types --no-deps
```
...
| Dependencies | jmap-types, jmap-jscontact-types, serde, serde_json only |
| Field names | Must match RFC 9610 exactly |
| Wire format | camelCase JSON — `#[serde(rename_all = "camelCase")]` |
| Test oracle | Hand-written JSON from RFC 9610 / RFC 9553 examples — never from code under test |
| Attribute order | `#[non_exhaustive]` → `#[derive(...)]` → `#[serde(...)]` on every type |
| Sloppy-Value | `Option<serde_json::Value>` for IETF-defined nested objects (RFC 9553 JSContact value shapes); see workspace AGENTS.md and crate PLAN.md §10 |

## Subagent Guidance

- Spawn subagents for parallel work on independent modules
  (`contact_card.rs` vs `address_book.rs`)
- Never spawn two subagents editing the same file
- Give each subagent the relevant RFC section explicitly
...
- Do not add dependencies beyond jmap-types, jmap-jscontact-types,
  serde, serde_json
- Do not add async, tokio, or axum
- Do not add method handler logic — types only
- Do not add fields or types not present in RFC 9610 / RFC 9553
