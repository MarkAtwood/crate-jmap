# Agent Instructions — jmap-sharing-client
...
*only* the spec content (RFC 9670 JMAP Sharing here; RFC 8621 in
mail-client).
...
JMAP Sharing extension client methods (RFC 9670) on top of
`jmap-base-client`. Implements `Principal/*` and `ShareNotification/*`
with the associated `/get`, `/changes`, `/set`, `/query`,
`/queryChanges` methods.
...
jmap-types
    └── jmap-base-client
            ...
            └── jmap-sharing-client  ← this crate (sibling)
                ↑ also depends on jmap-sharing-types for wire types
```
...
```bash
cargo fmt --all
cargo clippy -p jmap-sharing-client --tests -- -D warnings
cargo test -p jmap-sharing-client
RUSTDOCFLAGS="-D warnings" cargo doc -p jmap-sharing-client --no-deps --all-features
```
...
- `~/PROJECT/jmap-chat-spec/references/rfc9670.txt` — normative
  (JMAP Sharing) for all method signatures, arguments, and response
  fields
