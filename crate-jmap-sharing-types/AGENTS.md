# Agent Instructions — jmap-sharing-types
...
are *only* the spec content (RFC 9670 JMAP Sharing here; RFC 8621
in mail-types).
...
2. Read the relevant RFC 9670 section before implementing any type
...
JMAP Sharing extension data types (RFC 9670). Types only — no
method handlers. No async. Consumed by `jmap-sharing-server` and
`jmap-sharing-client`.
...
```
jmap-types
    └── jmap-sharing-types  ← this crate (sibling of canonical jmap-mail-types)
            ├── jmap-sharing-server
            └── jmap-sharing-client
```
...
```bash
cargo fmt --all
cargo clippy -p jmap-sharing-types -- -D warnings
cargo test -p jmap-sharing-types
RUSTDOCFLAGS="-D warnings" cargo doc -p jmap-sharing-types --no-deps
```
...
| Field names | Must match RFC 9670 exactly |
| Wire format | camelCase JSON — `#[serde(rename_all = "camelCase")]` |
| Test oracle | Hand-written JSON from RFC 9670 examples — never from code under test |
...
- Do not add fields or types not present in RFC 9670 unless
  explicitly directed
