//! Shared test helpers for jmap-calendars-client integration tests.
//!
...
///
/// Account: `A13824` / `john@example.com`, primary for `urn:ietf:params:jmap:calendars`.
/// Also includes `urn:ietf:params:jmap:principals:availability` for getAvailability tests.
///
...
pub fn make_session(server: &MockServer) -> jmap_base_client::Session {
    let json = serde_json::json!({
        ...
        "primaryAccounts": {
            "urn:ietf:params:jmap:calendars": "A13824",
            "urn:ietf:params:jmap:principals:availability": "A13824"
        },
...
/// Build a [`jmap_calendars_client::SessionClient`] pointed at the mock server.
///
...
pub async fn make_client(server: &MockServer) -> jmap_calendars_client::methods::SessionClient {
    ...
    client.with_calendars_session(make_session(server))
}
