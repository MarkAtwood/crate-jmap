...
pub use methods::{
    AddedItem, CalendarEventGetParams, CalendarEventParseResponse, CalendarSetParams,
    ChangesResponse, GetResponse, PrincipalGetAvailabilityResponse, QueryChangesResponse,
...
pub trait JmapCalendarsExt: sealed::Sealed {
    ...
    /// `urn:ietf:params:jmap:calendars` or whose `primaryAccounts` map
    /// has no entry for the calendars capability. The constructor
    ...
    /// `"no primary account for urn:ietf:params:jmap:calendars"`.
    ///
    /// Callers that want to guard at the binding site can pre-check
    /// the session before calling this method via
    /// [`session.primary_account_id("urn:ietf:params:jmap:calendars")`](jmap_base_client::Session::primary_account_id).
